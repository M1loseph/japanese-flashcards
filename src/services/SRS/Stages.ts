...
export const createSRSStages = (randomGenerator: () => number = Math.random): readonly SRSStage[] => {
    return [
        ...
        },
        {
            ...
            icon: IconInfinity,
            waitDuration: () => {
                const randomDaysToAdd = Math.floor(60 * randomGenerator());
                return dayjs.duration({ months: 8 }).add(dayjs.duration({ days: randomDaysToAdd }));
            },
        },
    ];
};

export const SRS_STAGES: readonly SRSStage[] = createSRSStages();
